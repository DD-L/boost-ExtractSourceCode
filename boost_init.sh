...
OUTPUT_DIR=$THISDOCDIR/boost
BOOST_SRC=$OUTPUT_DIR/boost_1_57_0
SUCCESS_FLAG=$BOOST_SRC/boost/success_flag
...
$UNPACK_TOOL x -y $BOOST_PACKAGE -o$OUTPUT_DIR 
if [ $? -eq 0 ]; then
	touch $SUCCESS_FLAG
fi
# check boost
...
if [ x"$ret" = "$SUCCESS_FLAG is missing..." ]; then
	echo "Err: checking $BOOST_SRC/boost ... no!"
	exit 1
fi
exit 0
